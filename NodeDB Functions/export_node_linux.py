...
def get_meshtastic_output(port):
    ...
    try:
        ...
        safe_port = port.replace('/', '_').replace(':', '')
        with open(f"meshtastic_raw_output_{safe_port}.txt", "w", encoding='utf-8', errors='replace') as f:
            f.write("STDOUT:\n")
            f.write(e.stdout or "")
            f.write("\n\nSTDERR:\n")
            f.write(e.stderr or "")
        print(f"⚠️ Raw output written to meshtastic_raw_output_{safe_port}.txt for inspection.")
        return ""
...
def clean_text(text):
    ...
    text = ''.join(c for c in text if 32 <= ord(c) <= 126 or unicodedata.category(c).startswith('So'))  # Keep emoji
    return text.strip()
...
def parse_meshtastic_table(output):
    ...
    headers = [col.strip() for col in header_line.split('│')][1:-1]  # Strip outer │ borders

    timestamp_now = datetime.now().date().isoformat()
    rows = []

    for line in data_lines[1:]:
        columns = [col.strip() for col in line.split('│')][1:-1]  # Keep empty columns, drop border
        if len(columns) == len(headers):
            ...
            print(f"⚠️ Skipping malformed row (expected {len(headers)} cols): {line.encode('utf-8', errors='replace').decode('utf-8')}")
